...
async def get_expired_paid_users() -> list[dict]:
    """
    Return users whose paid plan just expired (within last hour).
    Used to kick them from private group before downgrading.
    """
    pool = await get_pool()
    async with pool.acquire() as conn:
        rows = await conn.fetch("""
            SELECT user_id, plan, paid_until
            FROM users
            WHERE plan != 'free'
              AND paid_until IS NOT NULL
              AND paid_until < NOW()
              AND paid_until > NOW() - INTERVAL '1 hour'
        """)
        return [dict(r) for r in rows]

async def get_expiring_users(days: int = 3) -> list[dict]:
    """
    Return paid users whose subscription expires within `days` days.

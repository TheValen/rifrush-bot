...
PLAN_NAMES = {
    "hunter":        "🎯 HUNTER",
    "apex":          "⚡ APEX",
    "hunter_annual": "🎯 HUNTER Annual",
    "apex_annual":   "⚡ APEX Annual",
}
# Days per plan
PLAN_DAYS = {
    "hunter":        30,
...
# Base plan name (for DB plan field)
PLAN_BASE = {
    "hunter":        "hunter",
    "apex":          "apex",
    "hunter_annual": "hunter",
    "apex_annual":   "apex",
}
...
async def cb_upgrade(cb: CallbackQuery):
    await cb.message.edit_text(
        ...
        "⚡ <b>APEX</b>\n"
        "· Unlimited wallets · &lt;30s alerts · Smart signals\n\n"
        "💰 Pay in USDT, TON, BTC or ETH\n"
        "No KYC · No banks · Activates automatically",
        parse_mode="HTML",
        reply_markup=InlineKeyboardMarkup(inline_keyboard=[
            [InlineKeyboardButton(
                text="🎯 HUNTER — $19/mo",
                callback_data="pay:hunter"
            )],
            [InlineKeyboardButton(
                text="🎯 HUNTER Annual — $149/yr (save 35%)",
                callback_data="pay:hunter_annual"
            )],
            [InlineKeyboardButton(
                text="⚡ APEX — $49/mo",
                callback_data="pay:apex"
            )],
            [InlineKeyboardButton(
                text="⚡ APEX Annual — $399/yr (save 32%)",
                callback_data="pay:apex_annual"
            )],
            [InlineKeyboardButton(text="← Back", callback_data="back_main")],
        ])
    )
...
async def cb_pay(cb: CallbackQuery):
    ...
    try:
        async with aiohttp.ClientSession() as session:
            resp = await session.post(
                ...
                json={
                    ...
                    "description": f"Rifrush {PLAN_NAMES.get(plan, plan)} — {PLAN_DAYS.get(plan, 30)} days",
        ...
        if data.get("ok"):
            ...
            period    = f"{PLAN_DAYS.get(plan, 30)} days"
            suffix    = "/yr" if is_annual else "/mo"
            savings   = " (save 35%)" if plan == "hunter_annual" else " (save 32%)" if plan == "apex_annual" else ""

            await cb.message.edit_text(
                f"💳 <b>{PLAN_NAMES[plan]} — ${PLAN_PRICES[plan]}{suffix}{savings}</b>\n\n"
...
async def cmd_annual(msg: Message):
    ...
    await msg.answer(
        ...
        reply_markup=InlineKeyboardMarkup(inline_keyboard=[
            [InlineKeyboardButton(
                text="🎯 Hunter Annual — $149 (save 35%)",
                callback_data="pay:hunter_annual"
            )],
            [InlineKeyboardButton(
                text="⚡ Apex Annual — $399 (save 32%)",
                callback_data="pay:apex_annual"
            )],
            [InlineKeyboardButton(
                text="← Back",

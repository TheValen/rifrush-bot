...
from monitor import PLAN_THRESHOLDS
...
async def cmd_threshold(msg: Message, state: FSMContext):
    ...
    presets    = THRESHOLD_PRESETS[plan]
    preset_btns = [
    ...
    current = await get_user_threshold(msg.from_user.id) or PLAN_THRESHOLDS.get(plan, 500)

    await msg.answer(
        f"🎚 <b>Alert Threshold</b>\n\n"
        f"Plan: <b>{plan.upper()}</b> · Current: <b>${current}</b>\n"
...
async def cb_set_threshold(cb: CallbackQuery, state: FSMContext):
    ...
    if value == "custom":
        ...
        await cb.message.edit_text(
            f"✏️ Enter threshold in USD (min ${min_amount}):\n\n"
    ...
    await _save_threshold(cb, int(value), plan, min_amount)

@router.message(SetThreshold.waiting_amount)
async def process_threshold_amount(msg: Message, state: FSMContext):
    user = await get_user(msg.from_user.id)
    plan = get_plan(user)
    min_amount = 100 if plan == "hunter" else 1
    ...
    try:
        ...
        if amount < min_amount:
            await msg.answer(f"❌ Minimum is ${min_amount} for your plan. Try again:")
            return
    except ValueError:
        await msg.answer("❌ Enter a number, e.g. <code>500</code>", parse_mode="HTML")
        return

    await state.clear()
    await _save_threshold(msg, amount, plan, min_amount)

async def _save_threshold(event, amount: int, plan: str, min_amount: int):

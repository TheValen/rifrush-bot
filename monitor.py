...
from database import get_all_wallets, update_last_tx, check_and_expire_plans, get_user, get_user_threshold, get_expiring_users, get_expired_paid_users
...
async def start_monitor(bot: Bot):
    ...
    async with aiohttp.ClientSession() as session:
        ...
        while True:
            ...
            # Every hour — expire old plans + kick from group
            if iteration % 60 == 0:
                try:
                    expired = await get_expired_paid_users()
                    await check_and_expire_plans()
                    # Kick expired users from private group
                    if expired and PRIVATE_GROUP_ID:
                        from handlers import remove_from_private_group
                        for u in expired:
                            await remove_from_private_group(bot, u["user_id"])
                            logger.info(f"Kicked expired user {u['user_id']} from group")
                except Exception as e:

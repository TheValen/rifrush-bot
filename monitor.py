...
PUBLIC_CHANNEL_ID = os.getenv("PUBLIC_CHANNEL_ID", "")  # @RifrushAlerts
...
SPAM_TOKEN_PATTERNS = {
    "symbols": {
        ...
        "CMD", "POOL", "ZCHF", "XCHF",
        "DIXT", "FAM", "APE2", "PEPE2", "SHIB2", "DOGE2",
    },
    "name_keywords": [
        "airdrop", "claim", "visit", "reward", "bonus",
        "free", "win", "prize", "lottery", "giveaway",
        "cmd", "commander", "pool token",
        "dixt", "yefam", "inversebrah",
    ],
    # Spam function signatures in transaction data
    "spam_functions": [
        "atInversebrah",  # known spam contract pattern
        "Airdrop(",       # mass airdrop function
    ],
    # Zero address as sender = minted from nowhere = spam airdrop
    "zero_sender": "0x0000000000000000000000000000000000000000",
}

# Addresses known to send spam airdrops
SPAM_SENDERS = {
    "0x7b0da61eca7b4882a044",  # partial — CMD sender seen in logs
}
...
def is_spam_token(tx: dict) -> bool:
    ...
    # Zero value token tx = approve or spam with no real transfer
    ...
    # Spam function signatures
    fn_name = tx.get("functionName", "")
    for fn in SPAM_TOKEN_PATTERNS.get("spam_functions", []):
        if fn in fn_name:
            return True
...
def get_tx_usd_value(tx: dict) -> float:
    ...
    try:
        ...
        if tx_type == "token":
            ...
            # Unknown token — assign LOW signal level ($1)
            # so it passes threshold but shows as 🟢 SIGNAL not 🔴 WHALE
            # Real whales use ETH/stables, not obscure tokens
            return 1.0
...
# Global spam/filter counter (resets on restart)
_blocked_today = 0
...
def get_signal_level(usd_value: float) -> tuple[str, str]:
    """Return (emoji, label) based on USD value of transaction."""
...
def format_usd(value: float) -> str:
    """Format USD value nicely: $1.2M, $340K, $12.5K, $850"""
    if value >= 1_000_000:
...
def format_evm_alert(tx, address, chain, label):
    ...
    wallet_name = label if label else addr_short
    tx_hash     = tx.get("hash", "")
    tx_type     = tx.get("_tx_type", "native")
    ...
    counterpart = tx.get("from" if is_incoming else "to", "")
    ...
    if tx_type == "token":
        ...
        # USD estimate
        stable = {"USDT", "USDC", "DAI", "BUSD", "FRAX", "USDP", "TUSD", "USDD"}
        weth   = {"WETH", "ETH", "WBTC", "BTC"}
        if symbol.upper() in stable:
            usd_value = amount
            usd_str   = f" (~{format_usd(usd_value)})"
        elif symbol.upper() in weth:
            usd_value = amount * ETH_PRICE_USD
            usd_str   = f" (~{format_usd(usd_value)})"
        else:
            usd_value = get_tx_usd_value(tx)
            usd_str   = ""  # Don't show USD for unknown tokens
        signal_emoji, signal_label = get_signal_level(usd_value)

        return (
            f"{signal_emoji} <b>{signal_label} · {chain_label}</b>\n"
            f"━━━━━━━━━━━━━━━━━━━━\n"
            f"Wallet:  <b>{wallet_name}</b>\n"
            f"Action:  <b>{direction} {symbol}</b>\n"
            f"Amount:  <b>{amount:,.2f} {symbol}{usd_str}</b>\n"
            f"{'From' if is_incoming else 'To'}:     <code>{short_addr(counterpart)}</code>\n"
        ...
        price         = ETH_PRICE_USD if chain in ("eth", "base") else BNB_PRICE_USD
        usd_value     = amount * price
        signal_emoji, signal_label = get_signal_level(usd_value)
        usd_str = format_usd(usd_value) if usd_value > 0 else "—"

        return (
            f"{signal_emoji} <b>{signal_label} · {chain_label}</b>\n"
            f"━━━━━━━━━━━━━━━━━━━━\n"
            f"Wallet:  <b>{wallet_name}</b>\n"
            f"Action:  <b>{direction} {native_symbol}</b>\n"
            f"Amount:  <b>{amount:.4f} {native_symbol} (~{usd_str})</b>\n"
            f"{'From' if is_incoming else 'To'}:     <code>{short_addr(counterpart)}</code>\n"
            f"Network: {chain_label}\n"
            f"━━━━━━━━━━━━━━━━━━━━\n"
            f"<a href=\'{link}\'>🔍 View on Explorer</a>"
...
def format_sol_alert(signature, address, label):
    wallet_name = label if label else short_addr(address)

    return (
        f"🟡 <b>SIGNAL · Solana</b>\n"
        f"━━━━━━━━━━━━━━━━━━━━\n"
        f"Wallet:  <b>{wallet_name}</b>\n"
        f"Action:  <b>Transaction detected</b>\n"
        f"Network: Solana\n"
        f"━━━━━━━━━━━━━━━━━━━━\n"
...
CHANNEL_MIN_USD = 100_000  # $100K minimum to post to channel

async def post_to_public_channel(bot, alert_text: str, usd_value: float):
    """Post large whale alerts to public @RifrushAlerts channel."""
    if not PUBLIC_CHANNEL_ID:
        return
    if usd_value < CHANNEL_MIN_USD:
        return
    try:
        channel_text = (
            alert_text +
            "\n\n━━━━━━━━━━━━━━━━━━━━\n"
            "🔔 <a href='https://t.me/RifRushWhale_bot'>Track your own wallets → RifRush</a>"
        )
        await bot.send_message(
        ...
        logger.info(f"✅ Posted to public channel: ${usd_value:,.0f}")
    except Exception as e:
...
async def start_monitor(bot: Bot):
    ...
    iteration    = 0
    blocked_count = 0  # Count of filtered alerts
...
async def check_wallet(
    ...
    if new_tx_hash and alert_text:
        ...
        try:
            ...
            # Post large moves to public channel
            if tx_for_threshold is not None:
                usd_val = get_tx_usd_value(tx_for_threshold)
                await post_to_public_channel(bot, alert_text, usd_val)
            elif chain == "sol":
                # SOL — no USD value, always post if above min
                await post_to_public_channel(bot, alert_text, 999999)

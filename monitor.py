...
def format_evm_alert(tx, address, chain, label):
    ...
    if tx_type == "token":
        ...
        # USD estimate for stablecoins
        stable = {"USDT", "USDC", "DAI", "BUSD", "FRAX", "USDP", "TUSD", "USDD"}
        usd_value = amount if symbol.upper() in stable else get_tx_usd_value(tx)
        signal_emoji, signal_label = get_signal_level(usd_value)
        usd_str = f" (~{format_usd(usd_value)})" if usd_value < 999999 else ""
